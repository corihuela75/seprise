...
namespace Clinica_SePrise.TurnoE
{
    public partial class NuevoTurnoE : Form
    {
        ...
        {
            CargarTurnosDisponibles(cboEspecialidad.SelectedValue.ToString());
        }

        private void CargarTurnosDisponibles(string especialidad)
        {
            Conexion conexion = new Conexion();
            string query = "SELECT Id_turno, medico, fecha, hora_inicio, hora_fin, estado, especialidad FROM turnos WHERE" +
                "  estado = 'Disponible' AND especialidad = @especialidad";
            ...
            using (var connection = conexion.GetConnection())
            {
                try
                {
                    ...
                    dataGridViewTurnos.Columns["Medico"].HeaderText = "Médico";
                    ...
                    dataGridViewTurnos.Columns["Id_turno"].Width = 60;
                    dataGridViewTurnos.Columns["Medico"].Width = 150;
                    dataGridViewTurnos.Columns["fecha"].Width = 100;
                    dataGridViewTurnos.Columns["hora_inicio"].Width = 90;
                    dataGridViewTurnos.Columns["hora_fin"].Width = 90;
